...
AUD_WCD2_EP <- function(m,k,n_e,weight){
  n <- nrow(m)
  G <- m
...
################ 
AugUD_WCD <- function(X0, n, d, weight, inner_it=100, it=2)
{
  n_e <- nrow(X0)
  ...
  T0 <- 0.005*WCD2(m, weight)
  J <- min(50, 0.1*(n-n_e)*(n-n_e-1))
  crit <- NULL
  temp <- NULL
  proba <- NULL
  ...
  for (q in 1:it)      # outer loop
  {
    ...
    while(count<=inner_it)  # inner loop
    {
      ...
      g <- lapply(l, AUD_WCD2_EP, k = modulo+1, n_e = n_e, weight = weight)
