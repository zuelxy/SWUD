...
### max entropy
max_entropy <- as.matrix(read.table("BOR55_MaxEntropy.txt", header = F))
...
### GP_IMSE 
gp_imse <- as.matrix(read.table("BOR55_IMSPE.txt", header = F))
Aug_imse <- rbind(X0, gp_imse)
rownames(Aug_imse) <- 1:80
imse_y <- apply(Aug_imse, 1, borehole)

result_imse <- bore_score(Aug_imse, imse_y, borepoint, 
                          boreout, seed = 56)
