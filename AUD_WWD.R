...
AUD_WWD2_EP <- function(m,k,n_e,weight){
  n <- nrow(m)
  G <- m
...
################ 
AugUD_WWD <- function(X0, n, d, weight, inner_it=100, it=2)
{
  n_e <- nrow(X0)
  ...
  for (q in 1:it)      # outer loop
  {
    ...
    while(count<=inner_it)  # inner loop
    {
      ...
      g <- lapply(l, AUD_WWD2_EP, k = modulo, n_e = n_e, weight = weight)
